...
bool Parser::isKeyword(string s){
	...
	if(currentToken=="function"){
		currentTokenType = Tokens::KEYWORD;
...
string Parser::getToken(){
	...
	if(isDelimiter(expression[expressionIndex])){
		...
		expressionIndex++;
		currentTokenType = Tokens::DELIMITER;
		...
		currentTokenType = Tokens::NUMBER;
	}else{
		currentToken = "\0";
		return currentToken;
	}
	...
	if(token.isKeyWord(operatorIndex)){
		...
		if(operatorToken == "function"){
			...
			if(token.getToken(token.getSize()-1) != "{"){
				Tokens::stdError("syntax error for function definition, missing '{'");
				return  3;
			}else{
...
int Parser::getDelimiterPriorty() {
	...
	if (currentTokenType == Tokens::KEYWORD){
		return 40;
	}
